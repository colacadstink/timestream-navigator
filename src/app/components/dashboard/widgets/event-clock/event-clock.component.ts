  ...
  public loadEventInfo(refresh = false) {
    this.eventlink.getEventInfo(this.event.id, (refresh ? 'network-only' : undefined)).then((info) => {
      ...
      if(info.status === 'DRAFTING') {
        this.curTimerId = info.gameState?.top8DraftTimerID || info.gameState?.draftTimerID || undefined;
      }
      if(info.status === 'DECKCONSTRUCTION') {
        this.curTimerId = info.gameState?.constructDraftTimerID || undefined;
      }
      if(this.curTimerId) {
